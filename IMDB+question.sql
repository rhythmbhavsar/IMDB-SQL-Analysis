...
--   There are less number of movies released each year as compared to 2017

SELECT MONTH(date_published) AS month_num, COUNT(id) AS number_of_movies FROM movie GROUP BY month_num ORDER BY month_num ;
--   It is showing the same decling trend on a monthly basis as well
...
-- In 2019, a total of 1059 movies were produced either in the USA or India.

/* Both the USA and India collectively produced over a thousand movies in 2019. Delving into the Genre table promises an enjoyable exploration! Let's uncover the diverse genres within the dataset. */
...
-- A total of 3289 movies are exclusively associated with a single genre.
/*This quantity holds significance, indicating a considerable portion of the dataset.
Let's now explore the potential duration of RSVP Movies' upcoming project.*/
...
( SELECT genre, COUNT(movie_id)as movie_count, DENSE_RANK() OVER(ORDER BY COUNT(movie_id) DESC) genre_rank FROM genre GROUP BY genre ) 

SELECT * FROM thriller_genre_rank WHERE genre="Thriller";

--   Thriller has been ranked third with total 1484 movies
...
-- Dream Warrior Pictures and National Theatre Live are the production houses that have generated the highest number of hit movies, defined by an average rating exceeding 8.
-- Both companies hold the top rank, with a movie count of 3 each meeting this criterion.
...
--  'The Brighton Miracle' has highest average rating for the Drama genre
...
-- During the period between April 1, 2018, and April 1, 2019, a total of 361 movies were released, possessing a median rating of 8.
...
SELECT SUM(CASE WHEN name IS NULL THEN 1 ELSE 0 END) AS name_nulls, SUM(CASE WHEN height IS NULL THEN 1 ELSE 0 END) AS height_nulls, SUM(CASE WHEN date_of_birth IS NULL THEN 1 ELSE 0 END) AS date_of_birth_nulls, SUM(CASE WHEN known_for_movies IS NULL THEN 1 ELSE 0 END) AS known_for_movies_nulls 
FROM names
...
-- James Mangold, Joe Russo, and Anthony Russo emerge as the top three directors across the top three genres, with their movies achieving an average rating exceeding 8.
...
SELECT m.production_company, SUM(total_votes) AS total_vote_count, DENSE_RANK() OVER(ORDER BY SUM(total_votes) DESC) AS production_comp_rank FROM movie m INNER JOIN ratings r ON m.id = r.movie_id 
GROUP BY m.production_company 
ORDER BY total_vote_count DESC LIMIT 3

--   Marvel Studios has highest number of votes followed by Twentieth Century and Warner Bros
...
-- Vijay Sethupathi holds the position of the top actor, followed by Fahadh Faasil and Yogi Babu in the rankings.
...
-- Having the option to round values is beneficial but not mandatory, similar to sorting, which is also a useful feature but not obligatory.
...
--   Star Cinema & Twentieth Century box
